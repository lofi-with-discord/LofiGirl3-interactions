...
export default async function registUser (interaction: CommandInteraction, db: DatabaseClient, i18n: I18nParser, isFirstCall = false) {
  ...
  await interaction.editReply({ embeds: [embed], components: [{ components: [selectMenu], type: 1 }] }).catch(() => {})
  ...
  await localeSelection.deferReply().catch(() => {})
  ...
  if (isFirstCall) localeSelection.editReply({ content: i18n.__({ locale, phrase: 'locale_success_msg' }) }).catch(() => {})
  else localeSelection.editReply({ content: i18n.__({ locale, phrase: 'locale_success' }) }).catch(() => {})
